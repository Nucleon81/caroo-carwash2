...
import com.practice.project_1.service.UserService;
...
public class AuthController {
    ...
    static class UserCredentialsRequest {
        ...
        private Location userLocation;  // Updated: Now an object with latitude and longitude (doubles)
        private String mobileNumber;
    }
    ...
    private String generateJwtToken(String mobileNumber) {
        Users user = userRepository.findById(mobileNumber).orElseThrow(() -> new RuntimeException("User not found"));
        return Jwts.builder()
    ...
    public ResponseEntity<String> verifyOtp(@RequestBody OtpVerificationRequest request) {  // Change to OtpVerificationRequest
        String mobileNumber = normalizeMobileNumber(request.getMobileNumber());
        ...
        if (otpService.validateOtp(mobileNumber, request.getOtp())) {  // Use request.getOtp()
    ...
    public ResponseEntity<String> updateCredentials(@Valid @RequestBody UserCredentialsRequest request,
        ...
        logger.debug("Request data: name={}, email={}, location={}",
                request.getName(), request.getEmail(), request.getUserLocation());  // Updated logging for location
        try {
            ...
            user.setUserLocation(request.getUserLocation());  // Updated: Sets Location with lat/long
